...
pub enum LicenseError {
    ...
    InvalidDuration = 10,
    InvalidRights = 11,
    // #988 — Enrollment attestation errors.
    /// The caller did not supply an enrollment proof for the protected work.
    EnrollmentRequired = 12,
    /// The enrollment proof's `expires_at` is in the past at call time.
    EnrollmentExpired = 13,
    /// The proof nonce has already been consumed — replay detected.
    ProofReplayed = 14,
    /// The proof's `course_id` does not match the license's `work_id`.
    ProofCourseMismatch = 15,
    /// The proof's `learner` field does not match the transaction caller.
    ProofLearnerMismatch = 16,
}
