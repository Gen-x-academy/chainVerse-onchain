...
fn setup(now: u64) -> (Env, Address, Address, Address, Address, EscrowContractClient<'static>) {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.whitelist_token(&admin, &token_addr);

    (env, admin, buyer, seller, token_addr, client)
}
...
fn create_escrow_happy_path_stores_correct_record() {
    ...
    let escrow = client.get_escrow(&id).expect("escrow exists");
...
fn create_escrow_rejects_buyer_equals_seller() {
    let (_env, _admin, buyer, _seller, token, client) = setup(1_000);
...
fn create_escrow_rejects_non_whitelisted_token() {
    let (env, _admin, buyer, seller, _token, client) = setup(1_000);
